import styled from "styled-components";
import Heading from "../../ui/Heading";
import { Cell, Legend, Pie, PieChart, ResponsiveContainer, Tooltip } from "recharts";
import { useDarkMode } from "../../context/DarkModeContext";

const ChartBox = styled.div`
  background-color: var(--color-grey-0);
  border: 1px solid var(--color-grey-100);
  border-radius: var(--border-radius-md);
  padding: 2.4rem 3.2rem;
  grid-column: 3 / span 2;

  & > *:first-child {
    margin-bottom: 1.6rem;
  }

  & .recharts-pie-label-text {
    font-weight: 600;
  }

  @media (max-width: 768px) {
    padding: 2rem;
    grid-column: 1 / -1;
    width: 95%;
    margin: 0 auto;
    border-radius: var(--border-radius-lg);

    & > *:first-child {
      margin-bottom: 1.2rem;
      text-align: center;
    }
  }
`;

const SmallPieChart = styled(PieChart)`
  @media (min-width: 769px) {
    display: none;
  }
`;

const LargePieChart = styled(PieChart)`
  display: none;

  @media (min-width: 769px) {
    display: block;
  }
`;

const startDataLight = [
  {
    duration: "1 night",
    value: 0,
    color: "#ef4444",
    background: "#fff",
  },
  {
    duration: "2 nights",
    value: 0,
    color: "#f97316",
    background: "#fff",
  },
  {
    duration: "3 nights",
    value: 0,
    color: "#eab308",
    background: "#fff",
  },

  {
    duration: "4-5 nights",
    value: 0,
    color: "#84cc16",
    background: "#fff",
  },
  {
    duration: "6-7 nights",
    value: 0,
    color: "#22c55e",
    background: "#fff",
  },
  {
    duration: "8-14 nights",
    value: 0,
    color: "#14b8a6",
    background: "#fff",
  },
  {
    duration: "15-21 nights",
    value: 0,
    color: "#3b82f6",
    background: "#fff",
  },
  {
    duration: "21+ nights",
    value: 0,
    color: "#a855f7",
    background: "#fff",
  },
];

const startDataDark = [
  {
    duration: "1 night",
    value: 0,
    color: "#b91c1c",
    background: "#18212f",
  },
  {
    duration: "2 nights",
    value: 0,
    color: "#c2410c",
    background: "#18212f",
  },
  {
    duration: "3 nights",
    value: 0,
    color: "#a16207",
    background: "#18212f",
  },
  {
    duration: "4-5 nights",
    value: 0,
    color: "#4d7c0f",
    background: "#18212f",
  },
  {
    duration: "6-7 nights",
    value: 0,
    color: "#15803d",
    background: "#18212f",
  },
  {
    duration: "8-14 nights",
    value: 0,
    color: "#0f766e",
    background: "#18212f",
  },
  {
    duration: "15-21 nights",
    value: 0,
    color: "#1d4ed8",
    background: "#18212f",
  },
  {
    duration: "21+ nights",
    value: 0,
    color: "#7e22ce",
    background: "#18212f",
  },
];

function prepareData(startData, stays) {
  // A bit ugly code, but sometimes this is what it takes when working with real data 😅

  function incArrayValue(arr, field) {
    return arr.map((obj) => (obj.duration === field ? { ...obj, value: obj.value + 1 } : obj));
  }

  const data = stays
    .reduce((arr, cur) => {
      const num = cur.numNights;
      if (num === 1) return incArrayValue(arr, "1 night");
      if (num === 2) return incArrayValue(arr, "2 nights");
      if (num === 3) return incArrayValue(arr, "3 nights");
      if ([4, 5].includes(num)) return incArrayValue(arr, "4-5 nights");
      if ([6, 7].includes(num)) return incArrayValue(arr, "6-7 nights");
      if (num >= 8 && num <= 14) return incArrayValue(arr, "8-14 nights");
      if (num >= 15 && num <= 21) return incArrayValue(arr, "15-21 nights");
      if (num >= 21) return incArrayValue(arr, "21+ nights");
      return arr;
    }, startData)
    .filter((obj) => obj.value > 0);

  return data;
}

function DurationChart({ confirmStays }) {
  const { isDarkMode } = useDarkMode();
  const startData = isDarkMode ? startDataDark : startDataLight;
  const data = prepareData(startData, confirmStays);
  return (
    <ChartBox>
      <Heading as="h2">Stay duration summary</Heading>
      <ResponsiveContainer width="100%" height={240}>
        <LargePieChart>
          <Pie data={data} nameKey="duration" dataKey="value" innerRadius={85} outerRadius={110} cx="40%" cy="50%" paddingAngle={3}>
            {data.map((entry) => (
              <Cell key={entry.duration} fill={entry.color} stroke={entry.color} />
            ))}
          </Pie>
          <Tooltip />
          <Legend verticalAlign="middle" align="right" width="30%" layout="vertical" iconSize={15} iconType="circle" />
        </LargePieChart>
        <SmallPieChart>
          <Pie data={data} nameKey="duration" dataKey="value" innerRadius="45%" outerRadius="70%" cx="50%" cy="50%" paddingAngle={3}>
            {data.map((entry) => (
              <Cell key={entry.duration} fill={entry.color} stroke={entry.color} />
            ))}
          </Pie>
          <Tooltip />
          <Legend verticalAlign="bottom" align="center" layout="horizontal" iconSize={12} iconType="circle" height={36} />
        </SmallPieChart>
      </ResponsiveContainer>
    </ChartBox>
  );
}

export default DurationChart;
